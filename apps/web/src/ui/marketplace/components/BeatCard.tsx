...
const GENRE_STYLES: Record<string, { badge: string; hoverBorder: string }> = {
  CUMBIA: { badge: 'bg-[#1A2340] border border-[#C8860A]/40 text-[#C8860A]', hoverBorder: 'hover:border-[#C8860A]' },
  AFROBEAT: { badge: 'bg-[#1A7A6E]/20 text-[#1A7A6E]', hoverBorder: 'hover:border-[#1A7A6E]' },
  CHAMPETA: { badge: 'bg-[#B5651D]/20 text-[#B5651D]', hoverBorder: 'hover:border-[#B5651D]' },
  TRAP: { badge: 'bg-[#8B2500]/20 text-[#8B2500]', hoverBorder: 'hover:border-[#8B2500]' },
  'R&B': { badge: 'bg-[#1A7A6E]/20 text-[#1A7A6E]', hoverBorder: 'hover:border-[#1A7A6E]' },
  VALLENATO: { badge: 'bg-[#B5651D]/20 text-[#B5651D]', hoverBorder: 'hover:border-[#B5651D]' },
};

function getGenreStyle(genre: string) {
  return GENRE_STYLES[genre.toUpperCase()] ?? { badge: 'bg-surface-container-high text-on-surface-variant', hoverBorder: 'hover:border-[#C8860A]' };
}
  ...
  return (
    <div className={`bg-surface-container-low border border-primary/20 p-4 group ${genreStyle.hoverBorder} transition-all duration-500`}>
      ...
      <div className="relative aspect-square mb-6 overflow-hidden bg-surface-container-highest">
        ...
        >
          <span
            className="material-symbols-outlined text-sm flex transition-colors text-[#efe2c2]/40"
      ...
      <div className="space-y-4">
        <div>
          <span
            className={`${genreStyle.badge} text-[9px] px-2 py-0.5 font-display tracking-[0.2em] uppercase mb-2 inline-block`}
          >
            {beat.genre}
          </span>
