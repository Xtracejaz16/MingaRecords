...
const NAV_ITEMS = [
  {
    ...
    icon: 'storefront',
    active: true,
    activeClasses: 'text-white bg-[#8B2500]/30 border-l-4 border-[#8B2500]',
    hoverClasses: 'hover:bg-surface-container-highest/20 hover:text-[#1A7A6E]',
  ...
  },
  {
    ...
    active: false,
    activeClasses: '',
    hoverClasses: 'hover:bg-surface-container-highest/20 hover:text-[#C8860A]',
    badge: 0,
        ...
        {NAV_ITEMS.filter((item) => session || item.action !== 'profile').map((item) => (
          ...
          >
            ...
            {'badge' in item && item.badge > 0 && (
              <span className="ml-auto bg-[#8B2500] text-white text-[10px] px-1.5 py-0.5 rounded-full">
                {item.badge}
              </span>
            )}
          </button>
      ...
      {session && (
        <div className="mt-auto px-6 space-y-2">
          {FOOTER_ITEMS.map((item) => (
            <button
              key={item.label}
              className="w-full flex items-center gap-4 py-2 text-[#F2E8D0]/40 hover:text-[#F2E8D0] transition-colors text-xs font-display tracking-widest uppercase cursor-pointer"
              type="button"
