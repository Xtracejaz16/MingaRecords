      ...
      <main className="min-h-screen flex items-center justify-center pt-24 px-4">
        <section className="w-full max-w-lg mx-auto rounded-none auth-card-shell p-10 shadow-[0_0_40px_rgba(69,43,0,0.15)] border border-muiscaGold/20">
          <div className="space-y-5">
            {notice && (
              <div className="auth-feedback auth-feedback--neutral font-body text-xs">{notice}</div>
            )}
            ...
            <div className="flex gap-5 border-b border-muiscaGold/20">
              <button
                className={`tab-button w-full font-headline text-sm font-normal uppercase tracking-widest ${tab === 'login' ? 'is-active' : ''}`}
                type="button"
              ...
              <button
                className={`tab-button w-full font-headline text-sm font-normal uppercase tracking-widest ${tab === 'register' ? 'is-active' : ''}`}
                type="button"
            ...
            <div className="auth-emblem" aria-hidden="true">
              <svg viewBox="0 0 64 64" className="h-9 w-9 text-[#8b6a2a]" fill="none" stroke="currentColor" strokeWidth="2.5">
                ...
                <circle cx="21" cy="21" r="4" /><circle cx="43" cy="21" r="4" />
                <circle cx="21" cy="43" r="4" /><circle cx="43" cy="43" r="4" />
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              {tab === 'register' && (
                <div className="mb-8 grid grid-cols-2 gap-4">
                  <label className="group relative cursor-pointer">
                    <input type="radio" name="role" value="producer" className="peer hidden" checked={draft.role === 'producer'} onChange={() => handleChange('role', 'producer')} />
                  ...
                  <label className="group relative cursor-pointer">
                    <input type="radio" name="role" value="artist" className="peer hidden" checked={draft.role === 'artist'} onChange={() => handleChange('role', 'artist')} />
                    <div className="rounded-none border border-muiscaGold/30 p-4 text-center transition-all duration-300 peer-checked:border-muiscaGold peer-checked:bg-muiscaGold/5">
              ...
              <label className="field font-headline text-[10px] font-normal uppercase tracking-widest">
                ...
                <input className="w-full font-body text-base font-normal placeholder:italic placeholder:opacity-30" value={draft.identifier} onChange={(e) => handleChange('identifier', e.target.value)} placeholder="Ej: guerrero_zenu" type="text" />
              </label>

              {tab === 'register' && (
                <label className="field font-headline text-[10px] font-normal uppercase tracking-widest">
                  <span>Alias del proyecto</span>
                  <input className="w-full font-body text-base font-normal placeholder:italic placeholder:opacity-30" value={draft.alias} onChange={(e) => handleChange('alias', e.target.value)} placeholder="Tu nombre artístico" type="text" />
                </label>
              )}
              ...
              <label className="field field--password font-headline text-[10px] font-normal uppercase tracking-widest">
                ...
                <input className="w-full pr-12 font-body text-base font-normal placeholder:italic placeholder:opacity-30" value={draft.password} onChange={(e) => handleChange('password', e.target.value)} placeholder="••••••••" type={showPassword ? 'text' : 'password'} />
                <button className="field__toggle flex h-8 w-8 items-center justify-center" type="button" onClick={() => setShowPassword(!showPassword)}>
              ...
              <div className="flex items-center justify-between gap-5">
                <label className="checkbox">
                  <input type="checkbox" checked={draft.remember} onChange={(e) => handleChange('remember', e.target.checked)} />
                  <span className="font-body text-xs font-normal italic">Recordar mi esencia</span>
                </label>
                <button className="link-button font-body text-xs font-normal underline underline-offset-4" type="button" onClick={() => setFeedback('Recuperación pendiente.')}>
                  ¿Olvidaste tu llave?
                </button>
              </div>

              {feedback && <div className={`auth-feedback auth-feedback--${feedbackTone}`}>{feedback}</div>}
              ...
              <div className="grid gap-5">
                ...
                <button className="secondary-button primary-button--wide font-headline text-sm font-normal uppercase tracking-[0.2em]" type="button" onClick={() => handleTabChange(tab === 'login' ? 'register' : 'login')}>
